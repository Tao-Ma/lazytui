...
| item-action bar | theme border | **panes with item-actions** (the `containers` pane), left-anchored on the **bottom** border after `╰─` | the **focused** pane only, normal mode (suppressed in free-config); when wide enough (else it's hidden and the `N of M` count keeps the row) | a clickable row of per-item actions — `inspect logs shell stop restart kill`. Clicking one runs it against the **selected** container (`item_action` Msg). `inspect`/`logs`/`shell` mirror the `i`/`t`/`s` keys; the destructive `stop`/`restart`/`kill` run `docker <verb>` behind a y/N **confirm**. The `N of M` count coexists on the right when it fits. |
