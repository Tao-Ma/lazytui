...
| Edit in your editor | `e` on a files row, `:edit <path>`, or `:config` / `:config global` opens your editor (`editor:` config / `$VISUAL` / `$EDITOR` / `vi`) in an embedded terminal tab — auto-zoomed, back where you were on quit. An open doc tab showing the file refreshes on exit; config edits remind you they apply on the next start. |
