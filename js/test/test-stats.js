...
describe('[15] framework reconciles declared subscriptions (Model → Sub, #D13)', () => {
  ...
  it('_desiredSubs is a pure projection: app-global resize + the placed stats pane', () => {
    _place([STATS_PANE]);
    const desired = state._desiredSubs(getModel());
    // FIX-3 Phase 2: the app-global `resize` sub is always desired, plus the
    // stats pane's hub sub → two.
    eq(desired.size, 2, 'app-global resize + one stats hub sub');
