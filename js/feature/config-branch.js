...
function loadScript(branch, paths, excludes = []) {
  return [
    ...
    '    rm -rf "$p"',
    '    cp -a "$WT/$p" "$parent/"',
    '    # git records 0644/0755 only — files needing 0600 (SSH/OVPN',
    '    # inline keys, CA/TA keys, easy-rsa private dirs) come back',
    '    # world-readable and SSH refuses with UNPROTECTED PRIVATE KEY.',
    '    # Tighten the well-known sensitive names on the just-copied',
    '    # tree. Best-effort: failures (read-only mount) do not kill load.',
    '    find "$p" -type f \\( \\',
    '         -name "*.ovpn" \\',
    '         -o -name "id_ed25519" -o -name "id_rsa" -o -name "id_ecdsa" \\',
    '         -o -name "ssh_host_*_key" \\',
    '         -o -name "ca.key" -o -name "ta.key" \\',
    '         -o -path "*/pki/private/*" \\',
