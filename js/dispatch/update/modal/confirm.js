 ...
 * on `model.modal.continuation` (E14); `y` re-emits that Cmd, `n`/Esc clears.
 * No closure in the model.
...
function update(model, msg) {
  switch (msg.type) {
    ...
    case 'confirm_accept': {
      ...
      const next = _withModalMode(model, { confirmMode: false }, { confirm: { message: '' }, continuation: null });
      return [next, cmd ? [cmd] : []];
    }
    case 'confirm_reject':
      if (!model.modes.confirmMode) return [model, []];
      return [_withModalMode(model, { confirmMode: false }, { confirm: { message: '' }, continuation: null }), []];
    default:
      return [model, []];
  }
}
