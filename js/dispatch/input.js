...
function _handleWheel(mx, my, delta) {
  ...
  for (const p of allPanels()) {
    ...
    if (instanceKind(p.type) === 'detail') {
      ...
      const innerH = require('../leaves/geometry').getPanelViewportH(
...
function _mouseHandleFreeConfigMode(kind, mx, my, model) {
  ...
  const db = require('../leaves/geometry').visibleBoundsFor(getInstanceSlice('layout'), route.resolveViewerPaneId());
