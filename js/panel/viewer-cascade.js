...
const { setDetail, getSel } = require('../app/state');
const { getModel } = require('../app/runtime');
const {getPanelDef, getItems, getComponentSlice, getFocus } = require('./api');
const { getTabInfo, isTerminalTab, activeContentTab } = require('./viewer/tabs');
const { killCurrentProc, streamCommand } = require('../dispatch/action-runner');
...
function switchToTab(model, idx) {
  ...
  require('./api').dispatchMsg(require('./api').wrap('detail', { type: 'viewer_set_tab', tab: idx }));
  require('../dispatch/dispatch').applyMsg(model, { type: 'terminal_exit' });
  if (idx === 0) {
