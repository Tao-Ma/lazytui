...
function _clearUndoStacks() {
  require('../hosts/panel-host').dispatchMsg(require('./route').wrap('layout', { type: 'free_config_clear_undo' }));
}
...
function onMouseEvent(kind, mx, my) {
  const msg = kind === 'press'  ? { type: 'free_config_mouse_press',  mx, my, cols: cols() }
            ...
            : null;
  if (msg) {
    require('../hosts/panel-host').dispatchMsg(require('./route').wrap('layout', msg));
  }
}
